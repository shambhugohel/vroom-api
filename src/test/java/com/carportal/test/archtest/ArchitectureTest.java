...
/**
 * This class defines architectural and code-style constraints for the project `com.carportal` using
 * the Taikai library (a wrapper over ArchUnit-like checks).
 * <p>
 * It ensures consistency, enforces naming conventions, prevents unwanted dependencies, and
...
public class ArchitectureTest {
  ...
  void shouldFulfillConstraints() {
    Taikai.builder()
        .namespace("com.carportal") // The root package for all rules
        ...
        .test(test -> test
            .junit(junit -> junit
                .classesShouldNotBeAnnotatedWithDisabled() // Avoid @Disabled on test classes
                .methodsShouldNotBeAnnotatedWithDisabled())) // Avoid @Disabled on test methods

        // --- Spring-specific architectural rules ---
        .spring(spring -> spring
            .noAutowiredFields() // Enforce constructor injection over @Autowired fields

            .boot(boot -> boot
                .springBootApplicationShouldBeIn("com.carportal"))
