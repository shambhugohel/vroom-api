package com.carportal.test.archtest;
 ...
 * <p>
 * Extra architecture checks that complement existing Taikai checks. Each rule includes a short
...
public class AdditionalArchitectureTest {
  ...
  // -------------------------
  // Enforce controllers to be annotated with @RestController, be public and end with "Controller".
  // Public controllers are the conventional choice for Spring and avoid proxy/visibility surprises.
  @Test
  ...
  // -------------------------
  // Require @Service on service classes and enforce naming like "XxxService" or "XxxServiceImpl".
  @Test
  ...
  // -------------------------
  // Break cyclic dependencies across feature packages to keep modules independently maintainable.
  @Test
  void slices_should_be_free_of_cycles() {
    // matches com.carportal.<slice>.. and ensures no cyclic dependencies between slices
    slices().matching("com.carportal.(*)..")
  ...
  // This rule enforces that DTO classes only have final instance fields (no setters implied).
  @Test
  ...
  // -------------------------
  // Any class that extends java.lang.Exception or RuntimeException should end with "Exception"
  ...
  // -------------------------
  // Direct usage of System.out/err is noisy in server apps; prefer structured logging.
  @Test
  ...
  void production_code_should_not_depend_on_junit() {
    noClasses()
        .that().resideInAPackage("com.carportal..") // production namespace
  ...
  // -------------------------
  // Prevent methods from declaring or throwing plain Exception — prefer specific exceptions.
  @Test
  void methods_should_not_declare_generic_exception() {
    methods()
        .that().areDeclaredInClassesThat().resideInAPackage("com.carportal..")
